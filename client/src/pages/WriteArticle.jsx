...
const WriteArticle = () => {
  ...
  const [selectedItem, setSelectedItem] = useState(articleLength[0]);
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            articleLength.map((item,index)=> (
              <span onClick={()=>setSelectedItem(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedItem.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`} key={index}>{item.text}</span>
            ))
          }
        </div>
